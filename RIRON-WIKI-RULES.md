...
## RULE-07 出題実績テーブル：H18以降を5列（分野タグ付き）＋分野別集計＋傾向分析
...
<table className="data-table" style={{fontSize:"0.85rem"}}>
  ...
  <tbody>
    <tr><td>R07上</td><td>問XX</td><td>穴埋</td>
      <td><span style={{background:"#色コード",color:"white",padding:"1px 6px",borderRadius:"4px",fontSize:"0.75rem"}}>分野名</span></td>
      <td>具体的な出題内容（使用公式・判定対象・比較軸を明記）</td>
    </tr>
    {/* H18〜最新まで */}
  </tbody>
</table>
...
**分野タグの色ルール**: ページ内の主要トピックごとに固定色を割り当てる（3〜7分野程度）。色は `#e74c3c`(赤) `#2980b9`(青) `#27ae60`(緑) `#e67e22`(橙) `#8e44ad`(紫) `#1abc9c`(青緑) `#95a5a6`(灰) から選択。

**「何が問われたか」の記述ルール**:
- 旧: `「半導体素子」` → ❌ 曖昧すぎて分析不能
- 新: `「各種半導体素子（LED・フォトダイオード・ツェナー等）の動作原理と特徴」` → ✅ 具体的
- 計算問題: 使用公式・求める量を明記（例: `Av = -Rf/Ri 計算と出力電圧Voの符号判定`）
- 穴埋問題: 問われた用語・概念の範囲を明記（例: `ドナー・アクセプターと多数/少数キャリアの関係`）
- 論説問題: 比較軸・説明対象を明記（例: `BJT（電流制御）とFET（電圧制御）の構造の違い`）
...
```jsx
{/* 分野別出題分析 */}
<h3 style={{marginTop:"24px"}}>分野別出題傾向（H18〜R07 全N問）</h3>
<table className="data-table" style={{fontSize:"0.85rem",maxWidth:"600px"}}>
  <thead>
    <tr><th>分野</th><th>出題数</th><th>直近出題</th><th>出題周期</th></tr>
  </thead>
  <tbody>
    <tr><td><span style={{...}}>分野名</span></td><td><strong>N回</strong></td><td>R0X</td><td>周期の記述</td></tr>
    {/* 出題数降順で並べる */}
  </tbody>
</table>
```

**出題周期の記述例**: `ほぼ毎年（最頻出）` / `1〜2年に1回` / `2〜3年に1回` / `低頻度（優先度低）` / `初出題（今後注意）`

### 7-3. 傾向分析Callout

集計テーブルの直後に読み方ガイドを付ける：

```jsx
<Callout variant="tip" title="出題傾向の読み方">
  <strong>直近トレンド</strong>: 具体的なローテーション傾向・空白期間の分析。<br/>
  <strong>形式比率</strong>: 計算N回 vs 穴埋M回 vs 論説L回。対策の優先順位に直結。
</Callout>
```

**記載必須項目**:
1. 直近3〜5年のローテーション傾向（「次回出題の可能性が高い分野」を示唆）
2. 形式比率（計算/穴埋/論説）と対策への示唆
...
## RULE-34 marker 必須キーワード付与

**why**: 学習者がスマホで斜め読み（skimming）するとき、重要キーワードが視認できないと「何が核心か」が一瞬で判断できない。黄色マーカー `<span className="marker">...</span>` で核心フレーズを強調し、短時間学習での「抽出精度」を上げる。

### 対象キーワード種別

1. **定義文の核心語**：「〇〇は▲▲である」の▲▲部分（例：「PN接合は電流の一方通行弁」の「電流の一方通行弁」）
2. **比較表の判定軸**：正答判断のキー（例：「多数キャリアは電子」「多数キャリアは正孔」）
3. **支配因子・メカニズムの名称**：「仮想短絡」「仮想接地」「整流作用」「空乏層」など試験で頻出の専門用語
4. **数値定数**：VF=0.6V, Av=1 など覚えるべき数値（公式 `<Eq>` コンポーネント内ではなく、テキスト中に登場する場合）
5. **TrapBlock の正答側核心ワード**：`correct=` prop に含まれる場合、判定基準となるフレーズをマーク
6. **MinimumSet の rules / patterns**：「反転 → 出力の符号が逆」のような認識ルールのキー部分
7. **Callout の tip/note 内の結論フレーズ**：「ここが分かれば」の後の核心キーワード

### 禁止事項

- `<Eq tex="..."/>` コンポーネント自体には marker を付けない（数式は Eq タグで表示されるため重複不要）
- 1つの `<span>` に長文（40字超）を入れない。フレーズ単位（3〜15字）に絞る
- 既に `<strong>` タグのある箇所に重複してマーカーを付けることは避ける（どちらか一方で統一）
- 図説テキスト（figcaption）や例題の説明文には過度なマーカー付与は不要（重要度を見極める）

### 品質基準

| 重要度 | ページ例 | 最低マーカー数 |
|--------|--------|-----------|
| **A（毎年出題）** | SemiconductorPage（PN接合・整流作用・空乏層） | 最低18件 |
| **B（隔年出題）** | OpAmpPage（仮想短絡・仮想接地・反転増幅） | 最低12件 |
| **C（低頻度）** | その他ページ | 最低8件 |
...
```jsx
// NG: 長すぎるフレーズ
<span className="marker">PN接合ダイオードのアノード側が高電位のときに電流が流れる現象</span>

// OK: フレーズ単位
PN接合ダイオードのアノード側が<span className="marker">高電位</span>のときに
<span className="marker">電流が流れる</span>。

// OK: 定義の核心
「<span className="marker">PN接合 ＝ 電流の一方通行弁</span>」
```
...
| 2026-04-28 | RULE-34（marker 必須キーワード付与）追加。学習者の斜め読み対応・重要度別品質基準・実装例を定義 |
...
## RULE-46 テキスト折り返し・日本語改行品質

**why**: 日本語テキストに英字・数式・記号が混在すると、ブラウザのデフォルト改行アルゴリズムでは文字が切れる・意味不明な位置で折り返される。特にスマホ表示（RULE-30）で可読性に直結する。

### CSS 必須プロパティ

`.content` ブロックに以下を必ず保持する:
...
| プロパティ | 役割 |
|---|---|
| `overflow-wrap: anywhere` | 長い英字列・URL・数式がコンテナを超える場合のみ強制改行 |
| `word-break: auto-phrase` | 日本語フレーズ単位の改行（Chrome 119+）。非対応ブラウザではフォールバック |
| `line-break: strict` | CJK 句読点・小書き文字の禁則処理 |

### テーブルセル

`table.data td` / `table.data-table td` にも同じ3プロパティを設定する。テーブルセルは独立した書式文脈を形成するため `.content` からの継承だけでは不十分。

### インラインスタイルコンポーネント

`Callout` / `Analogy` / `FieldOverview` / `TrapBlock` の子テキスト `<div>` には最低限 `overflowWrap: "anywhere"` を設定する。

### FormulaTable 列幅

`公式:24% / 意味:32% / 使える条件:22% / 使えない条件:22%` を基本とする。意味列は日本語散文が入るため最大幅を確保する。

### 禁止

- `.content` から `overflow-wrap` / `word-break` / `line-break` を削除すること
- FormulaTable の意味列を 28% 未満に狭めること
```
