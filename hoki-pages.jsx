    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 14 }}>
          ...
          <MasteredToggle pageId="bshu-setsuchi" label="このページ" />
        </div>

        {/* 公式カード */}
        <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 'var(--radius)', padding: '14px 16px', marginBottom: 12 }}>
          <div style={{ fontSize: 11, color: 'var(--ink-3)', fontWeight: 600, letterSpacing: '0.05em', marginBottom: 6 }}>覚える公式（解釈17条）</div>
          <div style={{ fontSize: 20, fontWeight: 700, color: '#a06', fontFamily: 'serif', textAlign: 'center', padding: '10px 0', marginBottom: 10 }}>
            R_B ≤ 150 / Ig  [Ω]
          </div>
          <ul style={{ margin: 0, paddingLeft: 20, fontSize: 13, lineHeight: 1.85 }}>
            <li><strong>Ig</strong>：高圧電路の1線地絡電流 [A]（配電会社が提供または計算値）</li>
            <li>高圧電路が<strong>1秒以内</strong>に自動遮断される場合：<strong>300/Ig</strong> [Ω]</li>
            <li>高圧電路が<strong>0.5秒以内</strong>に自動遮断される場合：<strong>600/Ig</strong> [Ω]</li>
          </ul>
          <div style={{ marginTop: 10, padding: '8px 12px', borderLeft: '3px solid #c33', background: 'var(--bg-elev)', fontSize: 12, color: '#c33', fontWeight: 700 }}>
            ⚠ 遮断時間が<span style={{ color: '#c33' }}>短いほど許容抵抗値が大きくなる</span>（緩和方向）
          </div>
        </div>
        ...
        <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 'var(--radius)', padding: '12px 16px', marginBottom: 12 }}>
          ...
          <ol style={{ margin: 0, paddingLeft: 22, fontSize: 13, lineHeight: 1.9 }}>
            <li>「B種の抵抗値は固定値」→ 実際は <strong>Igによって変わる計算値</strong></li>
            <li>「遮断が速いほど抵抗値が小さくなる」→ 逆。速いほど <strong>上限値が大きく（緩和）</strong>なる</li>
            <li>「B種は低圧機器の保護」→ 実際は <strong>変圧器の高低圧混触防止</strong>（低圧側中性点に施設）</li>
          </ol>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          <button
            onClick={() => document.getElementById('exam-past')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}
            style={{ padding: '8px 16px', background: '#a06', color: '#fff', border: 'none', borderRadius: 'var(--radius)', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}
          >
            過去問形式へ →
          </button>
          <button
            onClick={() => document.getElementById('quick-review')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}
            style={{ padding: '8px 16px', background: 'var(--bg-elev)', color: 'var(--ink-2)', border: '1px solid var(--line)', borderRadius: 'var(--radius)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
          >
            1分復習 へ →
          </button>
          <button
      ...
      <div style={{
        ...
        <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--ink-2)', marginBottom: 8, letterSpacing: '0.05em' }}>📋 試験用 最短解法カード（B問題対策）</div>
        <ol style={{ margin: 0, paddingLeft: 22, fontSize: 13, lineHeight: 1.9 }}>
          <li><strong>遮断時間を確認</strong>：問題文で「1秒超」「1秒以内」「0.5秒以内」のどれかを読み取る</li>
          <li><strong>係数を選択</strong>：1秒超→150、1秒以内→300、0.5秒以内→600</li>
          <li><strong>Ig で割る</strong>：R_B(max) = 係数 / Ig</li>
          <li><strong>単位確認</strong>：IgはA（アンペア）、R_BはΩ</li>
        </ol>
        <div style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--line)' }}>
          💡 ゴール問題の正解：R_B = 150/5 = <strong>30 Ω</strong>（1秒超→係数150）
        </div>
      </div>
