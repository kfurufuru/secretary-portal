  ...
  return (
    <div>
      ...
      <ExamFocus items={[
        ...
        "tan を使った無効電力の求め方（tan = sinθ/cosθ）",
        "直列リアクトルの役割（高調波抑制・突入電流制限）",
        "コンデンサ電圧が系統電圧を超える理由（逆位相で差し引き → Vc > V）",
      ]} />

      <h2 id="abbrev">4. 略号と数値の整理</h2>
      <MemTable
        headers={["記号", "意味", "値・算出"]}
        rows={[
          ["V",    "系統線間電圧",                 "6,600V（問題で与えられる）"],
          ["XL",   "直列リアクトルのリアクタンス", "= 0.06Xc（k = 6%）"],
          ["Xc",   "コンデンサのリアクタンス",     "= 1/(ωC)、問題では比率で処理"],
          ["k",    "直列リアクトル比率",           "k = XL/Xc = 0.06（6%）"],
          ["Vc",   "コンデンサ端子電圧",           "= V / (1−k) = 6,600 / 0.94 ≒ 7,021V"],
          ["Q₁",  "改善前の無効電力",             "= 300 × (0.8/0.6) = 400 kvar"],
          ["Q₂",  "改善後の無効電力",             "= 300 × (0.6/0.8) = 225 kvar"],
          ["ΔQ",  "必要な無効電力差",             "= Q₁ − Q₂ = 175 kvar"],
          ["Qsc", "SC容量（kvar）",               "= ΔQ / (1−k) = 175 / 0.94 ≒ 186 kvar"],
        ]}
        note="tan(arccos 0.6) = 0.8/0.6、tan(arccos 0.8) = 0.6/0.8 は暗算で出せるようにする"
      />

      <h2 id="circuit">5. 直列リアクトル付進相コンデンサの等価回路</h2>
      <PlainExplain>
      ...
      <div style={{background: '#fff', border: '1px solid var(--line)', borderRadius: 'var(--radius)', padding: 16, marginBottom: 24}}>
        <div style={{fontSize: 12, color: 'var(--ink-3)', marginBottom: 10}}>▼ 図1：三相回路の接続図（SR + SC 直列枝 と 三相負荷 の並列回路）</div>
        <svg viewBox="0 0 820 300" style={{width: '100%', height: 'auto'}}>

          {/* 電源（左端） */}
          <circle cx="65" cy="155" r="42" fill="none" stroke="#555" strokeWidth="2"/>
          <text x="65" y="150" textAnchor="middle" fontSize="13" fontWeight="700" fill="#333">電源</text>
          <text x="65" y="167" textAnchor="middle" fontSize="11" fill="#666">6,600V</text>

          {/* 電源 → 上母線 */}
          <line x1="65" y1="113" x2="65" y2="70" stroke="#555" strokeWidth="2"/>
          <line x1="65" y1="70" x2="120" y2="70" stroke="#555" strokeWidth="2"/>

          {/* 電源 → 下母線 */}
          <line x1="65" y1="197" x2="65" y2="245" stroke="#555" strokeWidth="2"/>
          <line x1="65" y1="245" x2="120" y2="245" stroke="#555" strokeWidth="2"/>

          {/* 上母線 */}
          <line x1="120" y1="70" x2="760" y2="70" stroke="#333" strokeWidth="3"/>
          <text x="440" y="55" textAnchor="middle" fontSize="11" fill="#666">上母線（高圧側・6,600V）</text>

          {/* 下母線 */}
          <line x1="120" y1="245" x2="760" y2="245" stroke="#333" strokeWidth="3"/>
          <text x="440" y="265" textAnchor="middle" fontSize="11" fill="#666">下母線（帰還側）</text>

          {/* ── 枝①：SR + SC 直列（x=280） ── */}
          <circle cx="280" cy="70" r="5" fill="#27c"/>
          <line x1="280" y1="70" x2="280" y2="98" stroke="#27c" strokeWidth="2"/>

          {/* SR ボックス */}
          <rect x="193" y="98" width="174" height="52" rx="6" fill="#fde" stroke="#c66" strokeWidth="2"/>
          <text x="280" y="121" textAnchor="middle" fontSize="12" fontWeight="700" fill="#c66">SR（直列リアクトル）</text>
          <text x="280" y="138" textAnchor="middle" fontSize="11" fill="#c66">jXL = 0.06 × Xc</text>

          {/* SR → SC の接続 */}
          <line x1="280" y1="150" x2="280" y2="170" stroke="#27c" strokeWidth="2"/>
          <text x="376" y="162" fontSize="10" fill="#888">← 直列接続（同じ電流 I が流れる）</text>

          {/* SC ボックス */}
          <rect x="193" y="170" width="174" height="52" rx="6" fill="#e0eeff" stroke="#27c" strokeWidth="2.5"/>
          <text x="280" y="193" textAnchor="middle" fontSize="12" fontWeight="700" fill="#27c">SC（進相コンデンサ）</text>
          <text x="280" y="210" textAnchor="middle" fontSize="11" fill="#27c">−jXc（容量性）</text>

          <line x1="280" y1="222" x2="280" y2="245" stroke="#27c" strokeWidth="2"/>
          <circle cx="280" cy="245" r="5" fill="#27c"/>

          {/* 枝①の外ラベル */}
          <text x="180" y="125" textAnchor="end" fontSize="11" fontWeight="700" fill="#27c">進相コン</text>
          <text x="180" y="141" textAnchor="end" fontSize="11" fontWeight="700" fill="#27c">デンサ設備</text>

          {/* ── 並列接続の表示 ── */}
          <line x1="383" y1="157" x2="510" y2="157" stroke="#aaa" strokeWidth="1.5" strokeDasharray="5,3"/>
          <text x="447" y="149" textAnchor="middle" fontSize="12" fontWeight="700" fill="#888">‖ 並列 ‖</text>
          <text x="447" y="172" textAnchor="middle" fontSize="10" fill="#888">同じ母線電圧 V が印加</text>

          {/* ── 枝②：三相負荷（x=630） ── */}
          <circle cx="630" cy="70" r="5" fill="#555"/>
          <line x1="630" y1="70" x2="630" y2="110" stroke="#555" strokeWidth="2"/>

          {/* 負荷ボックス */}
          <rect x="508" y="110" width="244" height="108" rx="8" fill="none" stroke="#555" strokeWidth="2"/>
          <text x="630" y="145" textAnchor="middle" fontSize="14" fontWeight="700" fill="#333">三相負荷</text>
          <text x="630" y="168" textAnchor="middle" fontSize="12" fill="#666">有効電力 P = 300 kW</text>
          <text x="630" y="190" textAnchor="middle" fontSize="12" fill="#666">遅れ力率 cosφ = 0.6</text>
          <text x="630" y="208" textAnchor="middle" fontSize="10" fill="#999">（力率改善の対象）</text>

          <line x1="630" y1="218" x2="630" y2="245" stroke="#555" strokeWidth="2"/>
          <circle cx="630" cy="245" r="5" fill="#555"/>

          {/* 系統電圧 V のブレース（左端） */}
          <line x1="115" y1="70" x2="115" y2="245" stroke="#e87" strokeWidth="2"/>
          <line x1="109" y1="70" x2="121" y2="70" stroke="#e87" strokeWidth="2"/>
          <line x1="109" y1="245" x2="121" y2="245" stroke="#e87" strokeWidth="2"/>
          <text x="107" y="152" textAnchor="end" fontSize="12" fontWeight="700" fill="#e87">V</text>
          <text x="107" y="167" textAnchor="end" fontSize="11" fill="#e87">6,600V</text>
          <text x="107" y="182" textAnchor="end" fontSize="11" fill="#e87">印加</text>

        </svg>
      </div>

      {/* SVG2: 一相等価回路 */}
      <div style={{background: '#fff', border: '1px solid var(--line)', borderRadius: 'var(--radius)', padding: 16, marginBottom: 16}}>
        <div style={{fontSize: 12, color: 'var(--ink-3)', marginBottom: 10}}>▼ 図2：一相等価回路（SR と SC の直列接続）</div>
        <svg viewBox="0 0 720 200" style={{width: '100%', height: 'auto'}}>
          ...
          {/* 電源 */}
          <circle cx="70" cy="100" r="35" fill="none" stroke="#555" strokeWidth="2"/>
          <text x="70" y="96" textAnchor="middle" fontSize="13" fontWeight="700" fill="#333">E</text>
          <text x="70" y="112" textAnchor="middle" fontSize="10" fill="#666">= V（線間）</text>
          {/* 電源→SR */}
          <line x1="105" y1="78" x2="190" y2="78" stroke="#333" strokeWidth="2"/>
          {/* SR */}
          <rect x="190" y="65" width="110" height="30" rx="5" fill="#fde" stroke="#c66" strokeWidth="2"/>
          <text x="245" y="78" textAnchor="middle" fontSize="11" fontWeight="700" fill="#c66">SR（直列リアクトル）</text>
          <text x="245" y="91" textAnchor="middle" fontSize="10" fill="#c66">jXL</text>
          {/* SR→SC */}
          <line x1="300" y1="78" x2="390" y2="78" stroke="#333" strokeWidth="2"/>
          {/* SC */}
          <rect x="390" y="65" width="110" height="30" rx="5" fill="#e0eeff" stroke="#27c" strokeWidth="2.5"/>
          <text x="445" y="78" textAnchor="middle" fontSize="11" fontWeight="700" fill="#27c">SC（進相コンデンサ）</text>
          <text x="445" y="91" textAnchor="middle" fontSize="10" fill="#27c">−jXc</text>
          {/* SC→負荷端子 */}
          <line x1="500" y1="78" x2="600" y2="78" stroke="#333" strokeWidth="2"/>
          <circle cx="610" cy="78" r="5" fill="#555"/>
          <text x="625" y="82" fontSize="11" fill="#555">負荷側</text>
          {/* 下線（戻り） */}
          <line x1="105" y1="122" x2="610" y2="122" stroke="#333" strokeWidth="2"/>
          <circle cx="610" cy="122" r="5" fill="#555"/>
          {/* 電流 I の矢印 */}
          <line x1="155" y1="72" x2="185" y2="72" stroke="#e87" strokeWidth="2" markerEnd="url(#arr2)"/>
          <text x="160" y="62" textAnchor="middle" fontSize="10" fill="#e87" fontWeight="700">I →</text>
          {/* VL ブレース（上） */}
          <line x1="190" y1="50" x2="300" y2="50" stroke="#c66" strokeWidth="1.5"/>
          <line x1="190" y1="50" x2="190" y2="62" stroke="#c66" strokeWidth="1.5"/>
          <line x1="300" y1="50" x2="300" y2="62" stroke="#c66" strokeWidth="1.5"/>
          <text x="245" y="42" textAnchor="middle" fontSize="11" fill="#c66" fontWeight="700">VL = XL·I</text>
          <text x="245" y="56" textAnchor="middle" fontSize="10" fill="#c66">（電流より +90°）</text>
          {/* Vc ブレース（下） */}
          <line x1="390" y1="108" x2="500" y2="108" stroke="#27c" strokeWidth="2"/>
          <line x1="390" y1="96" x2="390" y2="110" stroke="#27c" strokeWidth="1.5"/>
          <line x1="500" y1="96" x2="500" y2="110" stroke="#27c" strokeWidth="1.5"/>
          <text x="445" y="130" textAnchor="middle" fontSize="11" fill="#27c" fontWeight="700">Vc = Xc·I</text>
          <text x="445" y="145" textAnchor="middle" fontSize="10" fill="#27c">（電流より −90°）</text>
          {/* E = Vc − VL */}
          <text x="360" y="175" textAnchor="middle" fontSize="12" fill="#333" fontWeight="700">E = Vc − VL　（逆位相の差し引き）</text>
          <text x="360" y="192" textAnchor="middle" fontSize="12" fill="#27c" fontWeight="700">→ Vc = E / (1 − XL/Xc) &gt; E</text>
        </svg>
      </div>

      {/* SVG3: フェーザ図 */}
      <div style={{background: '#fff', border: '1px solid var(--line)', borderRadius: 'var(--radius)', padding: 16, marginBottom: 24}}>
        <div style={{fontSize: 12, color: 'var(--ink-3)', marginBottom: 10}}>▼ 図3：フェーザ図（電流を基準にした電圧の位相関係）</div>
        <svg viewBox="0 0 820 300" style={{width: '100%', height: 'auto'}}>
          <defs>
            <marker id="arrPh" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="8" markerHeight="8" orient="auto">
              <path d="M0,0 L10,5 L0,10 z" fill="#444"/>
            </marker>
            <marker id="arrPhRed" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="8" markerHeight="8" orient="auto">
              <path d="M0,0 L10,5 L0,10 z" fill="#c66"/>
            </marker>
            <marker id="arrPhBlue" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="8" markerHeight="8" orient="auto">
              <path d="M0,0 L10,5 L0,10 z" fill="#27c"/>
            </marker>
            <marker id="arrPhGreen" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="8" markerHeight="8" orient="auto">
              <path d="M0,0 L10,5 L0,10 z" fill="#396"/>
            </marker>
          </defs>
          {/* 左パネル：フェーザ図 */}
          <rect x="10" y="10" width="380" height="280" fill="#fafbfc" stroke="#ccc" rx="6"/>
          <text x="200" y="35" textAnchor="middle" fontSize="13" fontWeight="700" fill="#333">フェーザ図（電流 I を基準）</text>
          {/* 原点 */}
          <circle cx="200" cy="160" r="4" fill="#444"/>
          {/* 電流 I（水平右向き：基準） */}
          <line x1="200" y1="160" x2="340" y2="160" stroke="#888" strokeWidth="2" strokeDasharray="5,3" markerEnd="url(#arrPh)"/>
          <text x="355" y="164" fontSize="11" fill="#888">I（基準）</text>
          {/* VL（電流より +90°：上向き） */}
          <line x1="200" y1="160" x2="200" y2="80" stroke="#c66" strokeWidth="2.5" markerEnd="url(#arrPhRed)"/>
          <text x="208" y="76" fontSize="12" fill="#c66" fontWeight="700">VL</text>
          <text x="208" y="90" fontSize="10" fill="#c66">（+90°）</text>
          {/* Vc（電流より −90°：下向き） */}
          <line x1="200" y1="160" x2="200" y2="260" stroke="#27c" strokeWidth="3" markerEnd="url(#arrPhBlue)"/>
          <text x="208" y="265" fontSize="12" fill="#27c" fontWeight="700">Vc</text>
          <text x="208" y="278" fontSize="10" fill="#27c">（−90°）</text>
          {/* E = Vc - VL（VLの先端からVcの先端への差分 → 下向き残り） */}
          {/* E は実際には Vc − VL = 下向き分だけ残る。等価的には Vc の先端から VL 分を引いた場所が E */}
          {/* 合成電圧 E（=印加電圧）は Vc より短く、同じ向き） */}
          <line x1="160" y1="160" x2="160" y2="220" stroke="#396" strokeWidth="3" markerEnd="url(#arrPhGreen)"/>
          <text x="128" y="192" textAnchor="middle" fontSize="12" fill="#396" fontWeight="700">E（合成）</text>
          <text x="128" y="207" textAnchor="middle" fontSize="10" fill="#396">= Vc − VL</text>
          {/* ブレース：VcとVLの差がE */}
          <line x1="178" y1="80" x2="178" y2="260" stroke="#aaa" strokeWidth="1" strokeDasharray="3,2"/>
          <line x1="172" y1="80" x2="184" y2="80" stroke="#aaa" strokeWidth="1"/>
          <line x1="172" y1="260" x2="184" y2="260" stroke="#aaa" strokeWidth="1"/>
          <text x="172" y="172" textAnchor="end" fontSize="10" fill="#555">|Vc|−|VL|</text>
          <text x="172" y="184" textAnchor="end" fontSize="10" fill="#555">= |E|</text>
          {/* 90°マーク */}
          <rect x="200" y="150" width="10" height="10" fill="none" stroke="#c66" strokeWidth="1"/>
          <rect x="200" y="160" width="10" height="10" fill="none" stroke="#27c" strokeWidth="1"/>
          {/* 右パネル：説明 */}
          <rect x="410" y="10" width="400" height="280" fill="#fafbfc" stroke="#ccc" rx="6"/>
          <text x="610" y="35" textAnchor="middle" fontSize="13" fontWeight="700" fill="#333">なぜ Vc &gt; E になるか</text>
          {/* 説明テキスト */}
          <text x="430" y="65" fontSize="12" fill="#333">① I が流れると：</text>
          <text x="445" y="85" fontSize="12" fill="#c66">VL = XL·I　（電流より +90°：上向き）</text>
          <text x="445" y="103" fontSize="12" fill="#27c">Vc = Xc·I　（電流より −90°：下向き）</text>
          <text x="430" y="125" fontSize="12" fill="#333">② VL と Vc は 180° 逆向き</text>
          <text x="430" y="147" fontSize="12" fill="#333">③ 印加電圧 E = Vc − VL（差し引き）</text>
          <text x="445" y="167" fontSize="12" fill="#333">E = Vc − 0.06·Vc = 0.94·Vc</text>
          <text x="430" y="195" fontSize="12" fill="#333">④ Vc について解くと：</text>
          <rect x="440" y="205" width="340" height="36" rx="6" fill="#e0eeff" stroke="#27c" strokeWidth="2"/>
          <text x="610" y="227" textAnchor="middle" fontSize="14" fontWeight="700" fill="#27c">Vc = E / 0.94 ≒ 1.064·E</text>
          <text x="610" y="255" textAnchor="middle" fontSize="12" fill="#333">コンデンサ電圧はシステム電圧の</text>
          <text x="610" y="272" textAnchor="middle" fontSize="13" fontWeight="700" fill="#d33">約 6.4% 大きくなる</text>
        </svg>
      </div>

      <h2 id="solve-a">6. 解き方（a）コンデンサ端子電圧</h2>
      <SolveFlow type="(a) 計算" steps={[
        "SR と SC が直列 → 一相等価回路で「電源電圧 E = Vc − VL」と設定",
        "XL = 0.06Xc を代入：VL = XL·I = 0.06Xc·I = 0.06Vc",
        "整理：E = Vc − 0.06Vc = 0.94Vc",
        "Vc = E / 0.94 に線間電圧 6,600V を代入（SR+SCは線間に接続）",
        "Vc = 6,600 / 0.94 ≒ 7,021V → 選択肢(5)",
      ]} />

      <h2 id="power-triangle">7. 力率三角形と無効電力の読み方</h2>
      <PlainExplain>
        <p>力率改善の計算は<strong>力率三角形</strong>で整理すると間違えにくい。P（有効電力）・Q（無効電力）・S（皮相電力）の三角形と、力率 cosφ・sinφ・tanφ の関係を把握する。</p>
      </PlainExplain>

      {/* SVG4: 力率三角形 */}
      <div style={{background: '#fff', border: '1px solid var(--line)', borderRadius: 'var(--radius)', padding: 16, marginBottom: 16}}>
        <div style={{fontSize: 12, color: 'var(--ink-3)', marginBottom: 10}}>▼ 図4：力率三角形（P・Q・S の関係）</div>
        <svg viewBox="0 0 820 280" style={{width: '100%', height: 'auto'}}>
          <defs>
            <marker id="arrTri" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto">
              <path d="M0,0 L10,5 L0,10 z" fill="#444"/>
            </marker>
          </defs>
          {/* 左：cosφ=0.6の三角形 */}
          <rect x="10" y="10" width="380" height="260" fill="#fafbfc" stroke="#ccc" rx="6"/>
          <text x="200" y="32" textAnchor="middle" fontSize="13" fontWeight="700" fill="#333">改善前（cosφ = 0.6）</text>
          {/* 三角形（直角は右下） */}
          {/* P（水平） */}
          <line x1="60" y1="200" x2="240" y2="200" stroke="#396" strokeWidth="3" markerEnd="url(#arrTri)"/>
          <text x="150" y="220" textAnchor="middle" fontSize="13" fontWeight="700" fill="#396">P = 300 kW</text>
          {/* Q（垂直） */}
          <line x1="240" y1="200" x2="240" y2="60" stroke="#e87" strokeWidth="3" markerEnd="url(#arrTri)"/>
          <text x="275" y="135" textAnchor="middle" fontSize="13" fontWeight="700" fill="#e87">Q₁ = 400 kvar</text>
          <text x="275" y="150" textAnchor="middle" fontSize="10" fill="#e87">= 300 × 0.8/0.6</text>
          {/* S（斜辺） */}
          <line x1="60" y1="200" x2="240" y2="60" stroke="#a06" strokeWidth="2.5"/>
          <text x="130" y="115" textAnchor="middle" fontSize="12" fill="#a06">S = 500 kVA</text>
          {/* 角度マーク */}
          <path d="M 80 200 A 20 20 0 0 1 72 186" fill="none" stroke="#333" strokeWidth="1.5"/>
          <text x="95" y="192" fontSize="11" fill="#333">φ₁</text>
          <text x="95" y="205" fontSize="10" fill="#333">cosφ=0.6</text>
          {/* 直角マーク */}
          <rect x="230" y="190" width="10" height="10" fill="none" stroke="#555" strokeWidth="1.5"/>
          {/* tanφ の矢印 */}
          <line x1="60" y1="168" x2="240" y2="168" stroke="#e87" strokeWidth="1" strokeDasharray="3,2"/>
          <text x="150" y="160" textAnchor="middle" fontSize="10" fill="#e87">tanφ₁ = Q/P = 0.8/0.6 = 1.333</text>

          {/* 右：cosφ=0.8の三角形 + 改善 */}
          <rect x="410" y="10" width="400" height="260" fill="#fafbfc" stroke="#ccc" rx="6"/>
          <text x="610" y="32" textAnchor="middle" fontSize="13" fontWeight="700" fill="#333">改善後（cosφ = 0.8）＋ コンデンサ効果</text>
          {/* P（水平、同じ長さ） */}
          <line x1="440" y1="200" x2="620" y2="200" stroke="#396" strokeWidth="3"/>
          <text x="530" y="220" textAnchor="middle" fontSize="13" fontWeight="700" fill="#396">P = 300 kW（変わらず）</text>
          {/* Q₂（改善後・垂直・短い） */}
          <line x1="620" y1="200" x2="620" y2="135" stroke="#2a8" strokeWidth="3" markerEnd="url(#arrTri)"/>
          <text x="680" y="170" textAnchor="middle" fontSize="12" fontWeight="700" fill="#2a8">Q₂ = 225 kvar</text>
          <text x="680" y="185" textAnchor="middle" fontSize="10" fill="#2a8">= 300 × 0.6/0.8</text>
          {/* ΔQ（Q₁→Q₂の差：コンデンサが供給する量） */}
          <line x1="620" y1="135" x2="620" y2="65" stroke="#e87" strokeWidth="2" strokeDasharray="4,2"/>
          <line x1="598" y1="135" x2="642" y2="135" stroke="#e87" strokeWidth="1.5"/>
          <line x1="598" y1="65" x2="642" y2="65" stroke="#e87" strokeWidth="1.5"/>
          <text x="680" y="102" textAnchor="middle" fontSize="12" fontWeight="700" fill="#e87">ΔQ = 175 kvar</text>
          <text x="680" y="117" textAnchor="middle" fontSize="10" fill="#e87">コンデンサが</text>
          <text x="680" y="129" textAnchor="middle" fontSize="10" fill="#e87">この分を供給</text>
          {/* S₂ */}
          <line x1="440" y1="200" x2="620" y2="135" stroke="#a06" strokeWidth="2"/>
          <text x="510" y="148" textAnchor="middle" fontSize="11" fill="#a06">S₂ = 375 kVA</text>
          {/* 直角マーク */}
          <rect x="610" y="190" width="10" height="10" fill="none" stroke="#555" strokeWidth="1.5"/>
          {/* 角度マーク */}
          <path d="M 460 200 A 20 20 0 0 1 455 185" fill="none" stroke="#333" strokeWidth="1.5"/>
          <text x="475" y="193" fontSize="11" fill="#333">φ₂</text>
          <text x="475" y="208" fontSize="10" fill="#333">cosφ=0.8</text>
        </svg>
      </div>

      {/* SVG5: 力率改善ビフォーアフター電流ベクトル図 */}
      <div style={{background: '#fff', border: '1px solid var(--line)', borderRadius: 'var(--radius)', padding: 16, marginBottom: 24}}>
        <div style={{fontSize: 12, color: 'var(--ink-3)', marginBottom: 10}}>▼ 図5：力率改善による電流の変化（同じ有効電力で電流が減る）</div>
        <svg viewBox="0 0 820 200" style={{width: '100%', height: 'auto'}}>
          <defs>
            <marker id="arrCur" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto">
              <path d="M0,0 L10,5 L0,10 z" fill="#e87"/>
            </marker>
            <marker id="arrCur2" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto">
              <path d="M0,0 L10,5 L0,10 z" fill="#2a8"/>
            </marker>
            <marker id="arrCurSc" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto">
              <path d="M0,0 L10,5 L0,10 z" fill="#27c"/>
            </marker>
          </defs>
          {/* 基準線（電圧V） */}
          <line x1="100" y1="100" x2="720" y2="100" stroke="#ccc" strokeWidth="1" strokeDasharray="4,2"/>
          <text x="730" y="104" fontSize="11" fill="#999">電圧V</text>
          {/* 改善前の電流 I₁（遅れ大） */}
          <line x1="100" y1="100" x2="210" y2="177" stroke="#e87" strokeWidth="3" markerEnd="url(#arrCur)"/>
          <text x="165" y="178" textAnchor="middle" fontSize="12" fontWeight="700" fill="#e87">I₁（改善前）</text>
          <text x="165" y="192" textAnchor="middle" fontSize="10" fill="#e87">cosφ=0.6・大</text>
          {/* 改善後の電流 I₂（遅れ小） */}
          <line x1="280" y1="100" x2="348" y2="148" stroke="#2a8" strokeWidth="3" markerEnd="url(#arrCur2)"/>
          <text x="360" y="148" textAnchor="middle" fontSize="12" fontWeight="700" fill="#2a8">I₂（改善後）</text>
          <text x="360" y="163" textAnchor="middle" fontSize="10" fill="#2a8">cosφ=0.8・小</text>
          {/* コンデンサ電流 Ic（進み・上向き） */}
          <line x1="460" y1="100" x2="460" y2="55" stroke="#27c" strokeWidth="2.5" markerEnd="url(#arrCurSc)"/>
          <text x="480" y="60" fontSize="12" fontWeight="700" fill="#27c">Ic（進相コンデンサ）</text>
          <text x="480" y="75" fontSize="10" fill="#27c">+90°（進み）で無効電力補償</text>
          {/* 合成矢印（I₁ + Ic = I₂） */}
          <text x="610" y="80" textAnchor="middle" fontSize="12" fill="#333">I₁ + Ic = I₂</text>
          <text x="610" y="95" textAnchor="middle" fontSize="11" fill="#333">遅れ無効分を相殺</text>
          <text x="610" y="120" textAnchor="middle" fontSize="13" fontWeight="700" fill="#2a8">→ 系統電流が小さくなる</text>
          <text x="610" y="136" textAnchor="middle" fontSize="10" fill="#555">（電線・変圧器の損失が減る）</text>
        </svg>
      </div>

      <h2 id="solve-b">8. 解き方（b）コンデンサ容量</h2>
      <SolveFlow type="(b) 計算" steps={[
        "改善前無効電力：Q₁ = 300 × (0.8/0.6) = 400 kvar（力率三角形のtanφ₁から）",
        "改善後無効電力：Q₂ = 300 × (0.6/0.8) = 225 kvar（力率三角形のtanφ₂から）",
        "必要な正味無効電力：ΔQ = Q₁ − Q₂ = 400 − 225 = 175 kvar",
        "SRがQsc × 0.06 を消費するため：Qsc × 0.94 = 175",
        "Qsc = 175 / 0.94 ≒ 186 kvar → 選択肢(3)",
      ]} />

      <PlainExplain>
        <p><strong>「SRがQsc × 0.06 を消費する」の導出</strong></p>
        <p>ここで間違えやすいのは <em>「なぜ SR の消費無効電力が Qsc × 0.06 になるのか」</em> という点。根拠は <strong>直列接続なので同じ電流 I が流れる</strong> ことに尽きる。</p>
        <p style={{fontSize:12, color:'var(--ink-3)', marginTop:4, marginBottom:8}}>
          ※ 三相全体では本来 Q = 3I²X だが、SC と SR で係数 3 は共通なので、比を見るために以下では省略する。
        </p>
        <table style={{width:'100%', borderCollapse:'collapse', fontSize:13, marginTop:4, marginBottom:12}}>
          <thead>
            <tr style={{background:'var(--bg-3)'}}>
              <th style={{border:'1px solid var(--line)', padding:'6px 10px', textAlign:'left'}}>素子</th>
              <th style={{border:'1px solid var(--line)', padding:'6px 10px', textAlign:'left'}}>無効電力</th>
              <th style={{border:'1px solid var(--line)', padding:'6px 10px', textAlign:'left'}}>意味</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px', color:'#27c', fontWeight:700}}>SC</td>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px', fontFamily:'serif'}}>Qsc = I² × Xc</td>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px'}}>コンデンサ単体の容量</td>
            </tr>
            <tr style={{background:'var(--bg-2)'}}>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px', color:'#c66', fontWeight:700}}>SR</td>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px', fontFamily:'serif'}}>QSR = I² × XL</td>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px'}}>リアクトルの消費無効電力</td>
            </tr>
            <tr>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px', color:'#c66'}}>SRが6%の場合</td>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px', fontFamily:'serif'}}>XL = 0.06Xc</td>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px'}}>6%リアクトル</td>
            </tr>
            <tr style={{background:'var(--bg-2)'}}>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px', fontWeight:700}}>よって</td>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px', fontFamily:'serif'}}>QSR = I² × 0.06Xc = <strong>0.06 × Qsc</strong></td>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px'}}>SRはSC容量の6%を消費</td>
            </tr>
            <tr style={{background:'#f0fff4'}}>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px', color:'#396', fontWeight:700}}>正味供給</td>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px', fontFamily:'serif', color:'#396', fontWeight:700}}>ΔQ = Qsc − QSR = <strong>0.94 × Qsc</strong></td>
              <td style={{border:'1px solid var(--line)', padding:'6px 10px', color:'#396'}}>系統に供給される無効電力</td>
            </tr>
          </tbody>
        </table>
        <div style={{background:'var(--bg-2)', border:'1px solid var(--line)', borderRadius:'var(--radius)', padding:'10px 16px', marginBottom:12, lineHeight:2.0, fontSize:13}}>
          <strong>直列なので電流が同じ</strong><br/>
          → <strong>無効電力の比 = リアクタンスの比</strong><br/>
          → <strong>SRのリアクタンスがSCの6%</strong><br/>
          → <strong>SRの消費無効電力もSC容量の6%</strong>
        </div>
        <p>したがって、系統へ正味で 175 kvar 供給したい場合は：</p>
        <div style={{textAlign:'center', fontSize:16, fontFamily:'serif', fontWeight:700, padding:'10px 0', color:'#27c'}}>
          0.94 × Qsc = 175　→　Qsc = 175 ÷ 0.94 ≒ <span style={{color:'#d33'}}>186 kvar</span>
        </div>
      </PlainExplain>
      ...
      <TrapTable traps={[
        ...
        { wrong: "SC容量 = ΔQ = 175 kvar（直列リアクトル分を無視）",            correct: "SRがQscの6%を消費するため Qsc = 175/0.94 ≒ 186 kvar（割り算）" },
        { wrong: "tan(arccos 0.6) = 0.6 （またはそのまま使えない）",            correct: "tan = sin/cos = 0.8/0.6 = 1.333。力率三角形で即計算（図4参照）" },
        { wrong: "改善前Q = P × cosφ = 300 × 0.6 = 180 kvar",                  correct: "Q = P × tanφ = P × sinφ/cosφ。P×cosφ は皮相電力の有効成分（= P自身）" },
        { wrong: "直列リアクトルの目的は力率改善",                               correct: "SRの目的は高調波抑制・突入電流制限。力率改善はSCが担う" },
        { wrong: "Qsc = ΔQ × (1 − 0.06) = 175 × 0.94（掛け算で計算）",         correct: "Qsc = ΔQ ÷ (1 − 0.06) = 175 ÷ 0.94 ≒ 186kvar（割り算）" },
      ]} />

      <h2 id="exam-r05-12">10. 過去問：R05下 問12 完全解答</h2>
      <ExamQuestion
        year="令和5年下期"
        qNum="12(a)"
      ...
      <ExamAnswer
        correct="(5) 7,021V"
        explanations={[
          { choice: "(1)", mark: "×", reason: "6,410V。Vcを系統電圧より小さく計算したパターン（SRを直列抵抗と混同）" },
          { choice: "(2)", mark: "×", reason: "6,795V。係数を誤って適用したパターン" },
          { choice: "(3)", mark: "×", reason: "6,807V。別の近似値を使用したパターン" },
          { choice: "(4)", mark: "×", reason: "6,995V。k=0.06でなく別の値を使用したパターン" },
      ...
      <SolveFlow type="解法 (b)" steps={[
        "改善前：Q₁ = 300 × tan(arccos 0.6) = 300 × (0.8/0.6) = 400 kvar",
        "改善後：Q₂ = 300 × tan(arccos 0.8) = 300 × (0.6/0.8) = 225 kvar",
        "必要な正味無効電力：ΔQ = 400 − 225 = 175 kvar",
        "SRがQsc × 0.06 を消費 → Qsc × 0.94 = 175 → Qsc = 175/0.94 ≒ 186 kvar",
        "→ (3) 186 kvar が正解",
      ]} />
      <ExamAnswer
        correct="(3) 186 kvar"
        explanations={[
          { choice: "(1)", mark: "×", reason: "170 kvar。SRの消費分を引いてしまった（逆方向の誤り）" },
          { choice: "(2)", mark: "×", reason: "180 kvar。丸め誤りまたは係数違い" },
          { choice: "(3)", mark: "○", reason: "186 kvar。175/0.94 ≒ 186.2 → 四捨五入" },
          { choice: "(4)", mark: "×", reason: "192 kvar。ΔQに対して別の係数を適用したパターン" },
          { choice: "(5)", mark: "×", reason: "208 kvar。力率改善の計算自体を誤ったパターン" },
        ]}
      />
      ...
      <UpdateLog entries={[
        { date: "2026-05-07", content: "v1.1: SVG5図追加（三相配置図・一相等価回路・フェーザ図・力率三角形・改善ビフォーアフター）・§記号→番号.形式", reason: "SVG不足・§記号禁止ルール対応" },
        { date: "2026-05-07", content: "v1.0: 初版（R05下問12対応・直列リアクトル等価回路・(a)(b)完全解法）", reason: "R05下問12 stub解消" },
      ]} />
