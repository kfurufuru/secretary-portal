# Knowledge Index

## AI活用

- [[ai-agent-architecture.md]] — AIエージェント アーキテクチャ完全ガイド（5層・Memory4層・エラー処理3パターン・あなたの環境対応表）(review, 2026-04-28)
- [[agentic-graphrag-architecture.md]] — Agentic GraphRAG アーキテクチャ (review, 2026-04-22)
- [[ai-gyomu-sentaku.md]] — AI化対象業務の策定フレームワーク（学習性×失敗許容度マトリクス） (published, 2026-04-22)
- [[cc-playbook.md]] — Claude Code プレイブック — 全17スライドのエッセンス (published, 2026-04-25)
- [[claude-long-edit-syntax-risk.md]] — Claude長編集での構文破壊リスクと検証プロトコル (published, 2026-04-23)
- [[ideas-from-news/2026-04-20.md]] — 製造現場AIファースト設計：UI廃棄戦略と工場音声IoT転用（2026-04-20蒸留） (published, 2026-04-20)
- [[learning-roadmap-filter.md]] — 学習ロードマップ判定フレームワーク（作る側/使う側 × ROI） (draft, 2026-05-01)
- [[ideas-from-news/2026-04-21.md]] — 設備AI活用の判断精度向上：xhigh使い分けルールとAI導入治験フェーズモデル（2026-04-21蒸留） (published, 2026-04-21)
- [[mcp-claude-integration.md]] — MCP — Claudeを実行AIに変える接続基盤 (review, 2026-04-22)
- [[promote-py-auto-promote-access-log.md]] — promote.py アクセスログベース自動昇格機能 (published, 2026-04-21)
- [[terminal-shell-cli.md]] — ターミナル・シェル・CLIの違い (published, 2026-04-22)
- [[top3-monday-api-key-401-error.md]] — top3-monday APIキー認証エラー（401）の対処手順 (published, 2026-04-21)

## AI活用 / Fカンパニー運用

- [[f-company-session-framework.md]] — Fカンパニー 高付加価値セッション設計フレームワーク (published, 2026-04-24)

## AI活用 / secretary運用

- [[claude-code-knowledge-pipeline.md]] — Claude Code 会話 → 知識昇格パイプライン (published, 2026-04-24)

## AI活用 / マネジメント

- [[business-skill-gap-analysis.md]] — ビジネススキルマップ ギャップ分析（AI社員3名の議論） (published, 2026-04-24)

## システム設計

- [[mkdocs-material-triple-nav-pattern.md]] — MkDocs Material 3重ナビ問題 — 一覧ページでの hide:toc パターン (published, 2026-05-01)
- [[portal-health-observability.md]] — Fカンパニーポータルの健全性可視化パターン（点→線化＋セルフ診断） (published, 2026-04-23)
- [[rtk-curl-output-bypass.md]] — rtk shell が curl 出力を自動フィルタする問題と回避策 (published, 2026-05-01)

## 未分類

- [[dataview-query-template.md]] — dataview-query-template (draft, 2026-04-23)

## 組織・マネジメント

- [[kozo-haaku-ryoku.md]] — 頭の良さの本質は「構造把握力」 (published, 2026-04-18)

_generated: 2026-04-25
